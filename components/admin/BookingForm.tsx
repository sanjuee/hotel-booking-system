    ...
    if (formData.checkInDate && formData.checkOutDate) {
      ...
      if (nights > 0) {
        ...
        if (isCallInMode && formData.roomId) {
          ...
          const selectedCat = roomCategories.find(
            (cat) => cat.id === formData.roomId
          )
        ...
        if (currentBasePrice > 0) {
          setFormData((prev) => ({
            ...prev,
            totalPrice: String(nights * currentBasePrice),
          }))
        }
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 sm:p-6">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        <div
          className={`flex justify-between items-center px-6 py-5 border-b transition-colors shrink-0 ${
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2
                className={`text-lg font-bold ${!isCallInMode && isWalkIn ? 'text-green-900' : 'text-slate-900'}`}
              >
                {isCallInMode
                  ? 'New Call-in Booking'
                  : isWalkIn
              ...
              <p
                className={`text-sm mt-0.5 font-medium ${!isCallInMode && isWalkIn ? 'text-green-700' : 'text-slate-500'}`}
              >
                {isCallInMode
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm shadow-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm shadow-sm text-slate-700 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2.5 bg-white border rounded-lg text-sm shadow-sm text-slate-700 outline-none ${maxCheckOutDate && formData.checkOutDate > maxCheckOutDate ? 'border-red-300 focus:border-red-500 focus:ring-1 focus:ring-red-500' : 'border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500'}`}
                />
                {maxCheckOutDate && (
                  <p className="text-[11px] text-red-500 mt-1.5 font-bold uppercase flex items-center gap-1">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm shadow-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                {isCallInMode ? (
                  <select
                    ...
                    className="w-full px-3 py-2.5 bg-white border border-slate-300 rounded-lg text-sm shadow-sm outline-none focus:border-blue-500 focus:ring-1"
                  >
                    <option value="">Select a category...</option>
                    {roomCategories.map((cat) => (
                      <option key={cat.id} value={cat.id}>
                        {cat.name} - Base: ₹{cat.price}
                      </option>
                    ))}
                  </select>
                ) : (
                  <input
                    type="text"
                    disabled
                    value={`Unit ${initialRoomNumber} (${initialRoomType})`}
                    className="w-full px-3 py-2.5 border border-slate-200 bg-slate-50 text-slate-500 font-medium rounded-lg text-sm shadow-inner cursor-not-allowed"
                  />
                )}
              </div>
                ...
                <div className="col-span-1 sm:col-span-2 p-4 bg-orange-50 border border-orange-200 rounded-xl flex items-start gap-3 mt-2 animate-in slide-in-from-bottom-2">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-orange-900">
                      Please use the Front Desk
                    </h4>
                    <p className="text-xs text-orange-700 mt-1 font-medium leading-relaxed">
                      Because this guest is arriving <b>today</b>, they need a
                      physical room assignment immediately. Please close this
                      modal, go to the{' '}
                      <b className="text-orange-900">Front Desk</b> tab,
                      and click an available green room to check them in.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="px-6 py-5 border-t border-slate-200 bg-slate-50 flex justify-end gap-3 shrink-0">
            <button
              type="button"
              onClick={() => setIsModalOpen(false)}
              className="px-4 py-2.5 bg-white text-slate-700 text-sm font-bold border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors shadow-sm"
            >
              Cancel
            </button>
            <button
