...
export async function POST(request: Request) {
  try {
    ...
    const { 
      guestName, email, phone, checkInDate, checkOutDate, totalPrice, 
      roomId,       // Sent by the Public form (Room Category)
      roomUnitId    // Sent by the Admin form (Specific Physical Room)
    } = body
    ...
    if (!assignedUnitId) {
      ...
      if (!roomId) {
        return NextResponse.json({ error: "Room category is required for public bookings" }, { status: 400 })
      }
      ...
      if (!availableUnit) {
        return NextResponse.json({ error: "Sorry, this room type is sold out for these dates." }, { status: 409 })
      }
